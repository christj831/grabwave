...
async function runYoutubeDownload(parsed, outDir, opts = {}) {
  ...
  if (parsed.type === 'youtube_playlist') {
     const out = await ytdlp.execPromise([parsed.url, '--dump-json', '--flat-playlist', '--cookies', 'cookies.txt']);
     const lines = out.trim().split('\n').filter(Boolean);
     for (const line of lines) {
       try {
         const info = JSON.parse(line);
     ...
     try {
       const out = await ytdlp.execPromise([parsed.url, '--dump-json', '--no-playlist', '--cookies', 'cookies.txt']);
