...
async function findYoutubeVideoUrl(baseQuery, durationMs, log) {
  const spotifySec =
  ...
  if (!fs.existsSync(cookiesPath)) {
    log('Warning: cookies.txt not found. YouTube searches may be less reliable.');
  }
  ...
  for (const q of queries) {
    try {
      ...
      try {
        ...
        log(`Executing yt-dlp with args: ${args.join(' ')}`);
        const p = ytdlp.execPromise(args, {}, ac.signal);
        out = await p;
      } finally {
        clearTimeout(timer);
      }
      ...
      for (const line of lines) {
        try {
          ...
          log(`Error parsing yt-dlp output line: ${line}. Error: ${e.message}`);
        }
      }
    } catch (e) {
      log(`yt-dlp search failed for query "${q}". Error: ${e.message}`);
    }
...
async function downloadYoutubeToMp3(youtubeUrl, outPath, onProgressCallback, signal) {
  ...
  '-x',
  '--audio-format', 'mp3',
  '--audio-quality', '320K', // 320 kbps CBR
...
export async function runDownload(spotifyUrl, outDir, opts = {}) {
  ...
  await asyncMap(tracks, 3, async (t, i) => {
    ...
    let n = 2;
    while (usedFilenames.has(outPath) || fs.existsSync(outPath)) {
      outPath = path.join(outDir, `${base} (${n}).mp3`);
      n++;
    }
    usedFilenames.add(outPath);
    ...
    while (!success && attempts < maxAttempts) {
      ...
      try {
        ...
        const ytUrl = await findYoutubeVideoUrl(query, t.durationMs, log);
        log(`    → ${ytUrl}`);
        await downloadYoutubeToMp3(ytUrl, outPath, null, signal);
    ...
    if (success) {
      ...
      onProgress({ type: 'endTrack', status: 'error', error: 'Failed after retries', ...trackDetails, ok, fail });
    }
  });
  ...
  await asyncMap(tracks, 3, async (t, i) => {
    ...
    while (!success && attempts < maxAttempts) {
      ...
      try {
        ...
        log(`    → ${t.url}`);
        await downloadYoutubeToMp3(t.url, outPath, null, signal);
        log(`    ✓ ${path.basename(outPath)}`);
        files.push(outPath);
        ok++;
...
async function asyncMap(array, concurrency, iteratee) {
  const results = [];
  const active = [];
  let currentIndex = 0;
  ...
  async function next() {
    ...
    const promise = iteratee(item, index, array);
    ...
    const wrappedPromise = (async () => {
      try {
        ...
        active.splice(active.indexOf(wrappedPromise), 1);
  ...
  await Promise.all(
    Array.from({ length: Math.min(concurrency, array.length) }, next)
  );
