...
export async function runDownload(spotifyUrl, outDir, opts = {}) {
  ...
  function updateProgress() {
    ...
    for (const [idx, state] of activeTracks.entries()) {
      if (idx < lowestIndex) {
        lowestIndex = idx;
        activeState = state;
      }
    ...
    if (activeState && activeState.details) {
      onProgress({ 
        type: activeState.percent === 0 ? 'startTrack' : 'trackProgress', 
        percent: activeState.percent, 
        ...activeState.details, 
        ok, 
        fail 
      });
  ...
  await asyncMap(tracks, 3, async (t, i) => {
    ...
    activeTracks.set(i, { details: trackDetails, percent: 0 });
    updateProgress();
      ...
      try {
        ...
        await downloadYoutubeToMp3(ytUrl, outPath, (prog) => {
          if (activeTracks.has(i)) {
            activeTracks.get(i).percent = prog.percent;
            updateProgress();
          }
        }, signal);
